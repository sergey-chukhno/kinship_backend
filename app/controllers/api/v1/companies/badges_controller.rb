# Company Badges API controller
# Handles badge assignment by company members
class Api::V1::Companies::BadgesController < Api::V1::Companies::BaseController
  include Pagy::Backend
  
  # POST /api/v1/companys/:company_id/badges/assign
  # Assign badge (admin/superadmin only for company dashboard)
  def assign
    # Verify company has active contract
    unless @company.active_contract?
      return render json: {
        error: 'Forbidden',
        message: 'Company must have an active contract to assign badges'
      }, status: :forbidden
    end
    ...
    recipient_ids.each do |recipient_id|
      begin
        ...
        user_badge = UserBadge.create!(
          ...
          project_title: params[:project_title] || "Badge assigned via Company Dashboard",
  ...
  # GET /api/v1/companys/:company_id/badges/assigned
  # List badges assigned by company members
  def assigned
    # Show all badges assigned by ANY company member (for company admin view)
    @user_badges = UserBadge.where(organization: @company)
