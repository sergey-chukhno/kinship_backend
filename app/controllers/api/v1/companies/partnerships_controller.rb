# Company Partnerships API controller
# Handles partnership management with companies/companys (superadmin only)
class Api::V1::Companies::PartnershipsController < Api::V1::Companies::BaseController
  include Pagy::Backend
  
  before_action :ensure_superadmin
  before_action :set_partnership, only: [:update, :destroy, :confirm, :reject]
  
  # GET /api/v1/companys/:company_id/partnerships
  # List all partnerships
  def index
    @partnerships = @company.partnerships.includes(:partnership_members)
  ...
  # POST /api/v1/companys/:company_id/partnerships
  # Create a new partnership (initiate partnership request)
  def create
    partnership_type = params[:partnership_type] || 'bilateral'
    partner_ids = Array(params[:partner_company_ids]) + Array(params[:partner_company_ids])
    ...
    ActiveRecord::Base.transaction do
      ...
      # Add initiating company as confirmed member
      ...
      # Add partner companys and send emails
  ...
  # PATCH /api/v1/companys/:company_id/partnerships/:id
  ...
  # DELETE /api/v1/companys/:company_id/partnerships/:id
  ...
  # PATCH /api/v1/companys/:company_id/partnerships/:id/confirm
  # Confirm partnership request (accept invitation)
  def confirm
    # Find the company's membership in this partnership
    ...
    unless partnership_member
      return render json: {
        error: 'Bad Request',
        message: 'No pending partnership request found for this company'
      }, status: :bad_request
    end
  ...
  # PATCH /api/v1/companys/:company_id/partnerships/:id/reject
  ...
  def set_partnership
    @partnership = @company.partnerships.find(params[:id])
  ...
  def serialize_partnership(partnership)
    partner_members = partnership.partnership_members.where.not(participant: @company)
